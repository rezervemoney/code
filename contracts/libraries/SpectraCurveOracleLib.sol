...
library SpectraCurveOracleLib {
    ...
    function getPTToIBTRate(address pool) internal view returns (uint256) {
        ISpectraPrincipalToken pt = ISpectraPrincipalToken(ICurveNGPool(pool).coins(1));
    ...
    function getPTToIBTRateSNG(address pool) public view returns (uint256) {
        ...
        if (maturity <= block.timestamp) {
            ...
            return pt.getIBTUnit().mulDiv(storedRates[1], storedRates[0]).mulDiv(
                IStableSwapNG(pool).price_oracle(0), CURVE_UNIT
            );
        }
    }
    ...
    function getYTToAssetRateSNG(address pool) internal view returns (uint256) {
        ISpectraPrincipalToken pt = ISpectraPrincipalToken(IStableSwapNG(pool).coins(1));
        uint256 ptToAssetRateCore = pt.previewRedeem(pt.getIBTUnit());
